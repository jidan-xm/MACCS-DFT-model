...
svr_params = {
    'C': 0.1,                  # 正则化参数
    'kernel': 'poly',          # 核函数类型
    'degree': 3,               # 多项式核的度数（仅对 poly 核函数有效）
    'gamma': 1,                # 核函数的系数
...
# 选择保存方法：CSV 文件
train_results_df.to_csv('D:/Tz/train_results.csv', index=False)
test_results_df.to_csv('D:/Tz/test_results.csv', index=False)
print("\n训练集结果已保存到: D:/Tz/train_results.csv")
print("测试集结果已保存到: D:/Tz/test_results.csv")

# 选择保存方法：Excel 文件（默认 openpyxl 引擎）
output_path = 'D:/Tz/svr_results.xlsx'
...
explainer = shap.KernelExplainer(svr.predict, X_train_scaled[:100])  # 使用部分训练数据作为背景数据
